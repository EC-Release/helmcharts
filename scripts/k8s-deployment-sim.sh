...
helm install k8s/agent+lber --set-file global.agtConfig=k8s/example/server+tls.env --generate-name
...
helm install k8s/example --set-file global.agtConfig=k8s/example/client+vln.env
